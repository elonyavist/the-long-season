...
Ultimo aggiornamento: 14 giugno 2026 — **fondazione tecnica sviscerata** in sessione di grilling: 27 decisioni di fondazione del motore raccolte in **Sezione J-bis** (Fase 0–2: schema attributi, modello del tempo, `GameDate`, ID, RNG, valore-attributo, match engine, `TeamStrength`, stato COW, generazione lega, stagione, verità statistica, schema `MatchEvent`, formato save, use-cases/CLI, scaffolding/enforcement, stati dinamici, `Money`, tattica, staff/scouting, vivaio+crescita, strutture, economia, mercato, match-day, narrativa/media+presidente, carriera manager). Tutte le aree-motore grigliate; marcate **TODO grill** (non bloccanti, Fase 2.2+): contratto UI↔motore (Area 17), onboarding/demo (18), locale-pack (20), business/IP (21). Documento chiuso, pronto per lo sviluppo. Nome di lavoro "The Long Season"; policy IP (Area 21); leve di ritenzione (K) e direzione d'arte retro-premium (L). **Prossimo passo: codice** — scheletro monorepo → `domain` → enforcement (checklist eseguibile in J-bis). Primo comando reale = `pnpm cli doctor` (non un commit di design); prima milestone gameplay utile = `pnpm cli simulate-season --seed=demo-001` (Step 1.2).
...
**Decisioni:**
- Scala attributi 0–20, interi visibili; progressi frazionari nascosti per la crescita.
...
- Potenziale: range scoutabile, mai un numero fisso. L'atterraggio nel range lo decide lo sviluppo reale: minutaggio, allenamento, strutture, personalità.
- Crescita e declino: curve per età con picchi diversi per ruolo; declino visibile e gestibile.
...
- Contropartite tecniche subito (giocatore + conguaglio): requisito critico — l'IA valuta i cartellini offerti col proprio fog of war e i propri bisogni; test anti-exploit nei batch (lo scambio del brocco non deve funzionare).
...
- valori di mercato: mediana e coda alta per categoria (controllo anti-inflazione su più stagioni)
...
- numero medio di trasferimenti per club a stagione; volume di mercato totale per categoria
...
### 4. ID stringa branded; `Record` è solo lookup; ordine sempre esplicito
**Decisione:** ogni ID è una **stringa branded namespaced non integer-like** (`club:perugia`, `comp:ita-3`, `p_000123`, `fx_000001`). Entità da content = slug semantici; entità generate = ID sequenziali deterministici (mai random). `Record<Id, Entity>` è **solo lookup table O(1)**: nessuna logica ordine-sensibile itera `Object.keys/values/entries`. Ogni ordine logico vive in **array espliciti di ID** (`GameState.clubIds`, `Club.playerIds`, `Round.fixtureIds`). Ogni `sort` ha un **tie-breaker finale deterministico** (`compareId`) e non muta l'array di input.
**Perché:** in JS le chiavi integer-like (`"123"`) si enumerano in ordine numerico crescente, non d'inserimento → baco di determinismo silenzioso. Stringa non-integer + disciplina d'iterazione lo eliminano (cintura + bretelle).
...
### 6. Valore-attributo: float valore-vero nel dominio, nebbia derivata
**Decisione:** `domain` memorizza i **valori veri precisi** (float 0–20 per abilità correnti e potenziale per-attributo). L'intero visibile (`floor`), i range fog-of-war a 4 livelli e il range di potenziale sono **calcolati da selector lato vista, mai persistiti**. È invece persistito lo **stato di conoscenza** (`ScoutingKnowledge`: club osservante, livello, data). L'hot-path del motore **evita funzioni trascendentali** (`Math.exp/sin/cos/log/pow` frazionario): le probabilità usano formule lineari, piecewise o lookup table.
**Perché:** rispetta "interi visibili, progressi frazionari nascosti, range calcolati e non salvati, nebbia solo lato utente" (Area 3) con una sola fonte di verità. Il rischio di determinismo non è il float salvato (IEEE754 ovunque, round-trip JSON pulito) ma le trascendentali, che possono divergere tra engine JS.
...
**Decisione:** la validazione confronta le distribuzioni simulate con `content/balance/calibration-targets.json` — pochi **benchmark aggregati** (gol/partita, %casa/pari/trasferta, punti primo/ultimo, spread forte-debole, upset rate, infortuni/cartellini quando esistono) scritti a mano per categoria, con bande di tolleranza (**larghe all'inizio**, niente finto rigore). `simulation-tools` produce un `CalibrationReport` PASS/FAIL → il cancello di Fase 1 diventa automatico/falsificabile. **Non sono dati reali importati**: nessuna identità, rosa, calendario, valore di mercato o database strutturato. *Fatti aggregati ≠ banca dati protetta* (dir. 96/9/CE: protetta la raccolta strutturata sostanziale, non i fatti). Il file si chiama `calibration-targets`, non `real-world-data`.
...
### 24. Mercato: valore vero vs percepito, trattativa a calendario, anti-exploit
**Decisione:** il mercato è una macchina di **informazione imperfetta, tempo e vincoli economici**. **Valore vero** (`playerTrueValue`, funzione data-driven di abilità/potenziale/età/contratto/categoria/ruolo) per debug/bilanciamento/simulazione — **mai mostrato all'utente** se non in debug. **Valore percepito** (`playerPerceivedValue(player, observingClubId)`) attraverso la `ScoutingKnowledge` del club valutante: chi non conosce il giocatore usa baseline conservativa o **consuma tempo scouting** prima di decidere → lo scouting diventa risorsa economica. **Trattativa = macchina a stati persistita avanzata dal calendario** (`TransferNegotiation`, `advanceNegotiations`), **due tavoli** (club: prezzo + formula; poi giocatore + procuratore: ingaggio/durata/bonus/commissione), entrambi possono far saltare il colpo; risposte in giorni-gioco, rilanci, club rivali, **deadline day** che comprime i tempi (tutto deterministico via `deriveRng`). **Formule = strutture di dominio** (`permanent`/`loan` con opzione/obbligo / `free_agent`); la `SellOnClause` (in `BasisPoints`) **persiste** sui diritti economici del giocatore e si liquida alla rivendita futura. **Anti-exploit contropartite** (requisito critico): l'IA accetta sse `valorePercepito(in entrata) + conguaglio + bisognoRuolo(cappato) ≥ valorePercepito(in uscita) × saleReluctance`, con `saleReluctance` moltiplicatore **esplicito** (titolare/giovane-top/simbolo/contratto-lungo → alto; scontento/esubero/scadenza → basso) **più i vincoli budget** (cash, wage budget, rate); se la contropartita non è nota e l'offerta è rilevante l'IA **richiede scouting** (la trattativa attende, non fallisce) → l'anti-exploit diventa tempo di mercato. `TransferRejectionReason` tipizzato per il feedback UI; eventi di trattativa = dati, mai testo (come `MatchEvent`, Decisione 13); test batch obbligatorio (lo scambio del brocco respinto ad alto tasso). **`Persona` unificato** (nome + tendenze leggere) condiviso da procuratori/presidenti/allenatori-IA. **Parametri zero = pilastro del mercato di C**: scadenza contratto → pool svincolati, firma = solo ingaggio + commissione + bonus, niente cartellino.
**Perché:** il fog (Decisioni 6, 20) entra direttamente nelle decisioni economiche → lo scouting paga, il brocco-swap fallisce, l'asimmetria informativa taglia in due sensi; `saleReluctance` esplicita serve perché il valore percepito da solo non cattura "è il mio capitano, non lo cedo a quel prezzo".
...
- `valuation-curves.json` — curve di valore giocatore (abilità/potenziale/età/contratto/categoria/ruolo) (Decisione 24)
...
Le versioni di questi file (`roleWeightsVersion`, `generationConfigVersion`, `matchEngineConfigVersion`, `calibrationTargetsVersion`, `playerStateCurvesVersion`, `rngAlgorithmVersion`, oltre a `eventSchemaVersion` e `saveSchemaVersion`) sono tracciate in `GameMeta`/nei report batch e nel save, perché cambiarle sposta le distribuzioni o richiede migrazione.
...
3. **Domain types**: `domain/src/types/ids.ts` (branded), `value-objects/game-date.ts` (epoch-day), `value-objects/money.ts` (+ `BasisPoints`), `value-objects/rating.ts` (`AbilityValue`).
