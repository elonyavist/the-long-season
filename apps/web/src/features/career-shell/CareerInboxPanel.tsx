...
export function CareerInboxPanel({ view, text, onActionClick }: CareerInboxPanelProps): React.JSX.Element {
  return (
    <section
      className="tls-dashboard-inbox"
      aria-label={text("career.inbox.title")}
      data-action-required={view.actionRequiredCount > 0}
      data-testid="career-inbox-panel"
    >
      <header className="tls-dashboard-inbox-header">
        <div className="tls-dashboard-inbox-title">
          <span aria-hidden="true" className="tls-dashboard-inbox-led" />
          <h2>{text("career.inbox.title")}</h2>
        </div>
        <dl className="tls-dashboard-inbox-counts">
          <div>
      ...
      {view.messages.length === 0 ? (
        ...
        <ul className="tls-dashboard-inbox-list">
          {view.messages.map((message) => (
            <li
              className="tls-dashboard-inbox-message"
              data-action-required={message.actionRequired}
            ...
            >
              <div className="tls-dashboard-inbox-meta">
                ...
                <span className="tls-dashboard-inbox-badge">{text(priorityLabelKey(message.priority))}</span>
                <span className="tls-dashboard-inbox-badge">{text(statusLabelKey(message.status))}</span>
              </div>
              <strong>{text(message.titleKey as MessageKey)}</strong>
              <p>{text(message.summaryKey as MessageKey)}</p>
              {message.relatedLabels.length === 0 ? null : (
                <ul className="tls-dashboard-inbox-related">
                  {message.relatedLabels.map((label) => (
