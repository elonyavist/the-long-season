...
## Known blind spot in this project (upstream #1666)

Every one of the eight shipped balance calibrations produces zero nodes and is **absent from this graph**:
...
The extractor reports them as `produced zero nodes` on every run. Consequence: `query`, `explain`, and `affected` cannot see which coefficient a symbol reads, and `affected "<SomeCalibrationConfig>"` lists the TypeScript consumers while silently omitting the JSON that actually ships the numbers. Graph silence about a balance value is a blind spot, never evidence. Read the JSON.

This section is local to this project and `graphify install` overwrites this file. If it disappears after an install or upgrade, restore it.
...
**In this project, prefer `explain` over `query` whenever the question names a symbol.** Measured here: `query "which function generates players"` returned 283 nodes, truncated to 18, mixed UI and i18n into an engine question, and surfaced one of three callers rather than the factory. `graphify explain "assembleGeneratedPlayer"` returned the answer with all 15 typed edges in 20 lines. Use `query` only when no symbol name is known, and treat its output as a lead to feed `explain`. Full rules in this repo's `CLAUDE.md`.
