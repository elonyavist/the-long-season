...
test("a missing task allocation is rejected", () => {
  const raw = structuredClone(matchTacticsCalibrationJson) as Record<string, unknown>;
  const shape = raw["tacticalShape"] as Record<string, Record<string, Record<string, number>>>;
  delete shape["taskAllocationBasisPointsByRole"]?.["striker"]?.["build_up"];
...
test("a structurally valid asset that breaks a football invariant is still rejected", () => {
  const raw = structuredClone(matchTacticsCalibrationJson) as Record<string, unknown>;
  const shape = raw["tacticalShape"] as Record<string, Record<string, Record<string, number>>>;
  const goalkeeper = shape["taskAllocationBasisPointsByRole"]?.["goalkeeper"];
...
test("a parseable allocation row that overspends its budget is rejected by policy", () => {
  const raw = structuredClone(matchTacticsCalibrationJson) as Record<string, unknown>;
  const shape = raw["tacticalShape"] as Record<string, unknown>;
  const byRole = shape["taskAllocationBasisPointsByRole"] as Record<string, Record<string, number>>;
  const striker = byRole["striker"] as Record<string, number>;
  striker["counter_threat"] = (striker["counter_threat"] as number) + 1;
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof MatchTacticsCalibrationValidationError
      && error.message.includes("allocates 42001, expected 42000"),
  );
});
