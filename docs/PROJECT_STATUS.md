...
- Step: `docs/steps/34-match-event-concentration-rework/04-long-run-smoke-gate.md`
- Status: Blocked.
- Last verification: Step 04 smoke gate `50` worlds x `10` seasons with `seed-prefix=phase34-concentration` wrote `docs/audits/MATCH_EVENT_CONCENTRATION_LONG_RUN_REPORT.md` and failed with `2` failed worlds. The targeted creator-concentration metrics no longer fail: no failing `top_creator_goal_share_max`, `top_three_creator_goal_share_max`, or `top_assist_max` checks appeared in the smoke gate. The only failing check is `table_points_spread_avg` in `phase34-concentration-world-00003` (`29.1`) and `phase34-concentration-world-00040` (`29.7`). Strict `calibration-v1` balance still passes with goals per match `3.001`.
- Next action: Do not run Step 05 yet. Decide and document a narrow table-spread anomaly review/rework, or explicitly move `table_points_spread_avg` out of the Phase 34 concentration gate before attempting the final 250 worlds x 30 seasons run.
- Blocker: Phase 34 is blocked by smoke-gate table-spread failures that are outside the original creator-concentration fix scope; final 250x30 gate was not attempted.
