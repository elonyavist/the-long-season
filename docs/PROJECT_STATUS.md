...
| 11, 12 - season recap instrument, then `5 x 20` inspection | Not started. New; they run **before** 13 and 14, which need their shape baseline. Design: `docs/audits/PHASE_81_SEASON_RECAP_DESIGN.md` |
| 13 - diagnostics and integrated gates | Not started. A7 deadline |
| 14 - formation as a counter-move | Not started. Step 09 met its first prerequisite; Step 10 added a second - a manager cannot yet see what the opponent's shape is doing to him |
...
- Next action: Step 11. **File order is execution order.** Step 13 is the A7
  deadline and runs after the recap, so it is not judging the engine blind.
  Nothing in the UI reads Step 07's `route`, `routeCounts` or `shooterCounts`.
...
- Phase 81 Step 15 owns the checkpointed `50 x 20`, seven workers: engine
  ...
  Steps 14 and 15 above are unrelated.
...
- **A10 permits exactly one `5 x 20` inspection run before it** (Step 12), on a
  hard condition: never evidence, changes no calibration, widens no band, cited
  by nothing as measurement. An aggregate cannot show whether strikers or centre
  backs scored the goals. No cohort anywhere else.
...
third time: out of band there, the rule says reopen Step 06.
...
report, not the `750`-world distribution, which nobody re-runs before Step 15.
...
14 owns raising it to `~0.047` as a *counter-move* reward, on Step 12's shape
baseline, and a manager still cannot see what to counter.

### Frozen Tactical Baseline (Step 01)

`docs/audits/PHASE_81_TACTICAL_SHAPE_BASELINE.md` is the before-state and is not
regenerated. Amending a `TACTICAL_SHAPE_THRESHOLDS` value takes a numbered
amendment.
...
- Five routes, frozen here because Step 01 froze none: `central`, `left`, `right`,
  ...
  written down, read by the mirror invariant and the flank matchup alike.
- **Open: the flank claim has an instrument and no population.** Step 07A found
  ...
  This reopen needs a lopsided side; Step 10's sweep builds them.
...
- `TACTICAL_SHAPE_TASK_KIND` decides where it acts. `coordination` tasks scale
  ...
  synthesised eleven must therefore hold natural positions**, or every
  coordination task is suppressed and both presence tasks are not - a uniform
  bias that cost Step 10 a whole reference. `POSITION_SUITABILITIES` is the one
  canonical order; anything ranking by it walks it.
...
- **Only a load can prove a save.** `route` was absent from `match_events` for a
  ...
  mapper validates the unions rather than casting. Match-event schema `8`
  (`ShotContext.route`, absent for a penalty); explanation-trace schema `2`.
...
- **Shape on `structuralScore`, eleven on `score`.** Only ability and fit choose a
  ...
  a tired defender change a formation for a week, leaving 14 nothing to counter.
...
- Frozen presentation policy, in `@game/ui` because that is where the step
  ...
  displayed population - the other `14` reach AI clubs only. One country, one
  calibration version: a retune re-derives them, caught by the sweeps in
