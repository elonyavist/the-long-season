...
Build the first post-load career dashboard view from existing career state and
explicit caller inputs.

The builder should concentrate career-screen data mapping in one readable Module
so future UI code does not duplicate career-summary and matchday-readiness logic.
The app entry screen is intentionally simpler and should not require career
state.
...
- Derive only first career-screen facts from the current career state, such as:
  - selected club;
  - current date;
  - current season;
...
- Do not derive economics, salaries, contracts, stadium, or ticket-price facts.
...
- A single builder produces the first post-load career dashboard view.
