...
It does not attempt to render the full app entry menu because the entry menu is
a web shell concern; Phase 48 only needs to prove that entry/settings contracts
and post-load dashboard facts are structured.
...
- Do not render salaries, player contracts, stadium data, ticket prices, or
  economics summaries.
...
- `--dashboard` renders the first post-load career-screen facts from the shared
  view contract.
